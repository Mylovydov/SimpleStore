...
class UserProductController {
   ...
   async getAll (request, response) {
      try {
         ...
         return response.json({ allTagTypes, allTags, allProducts, productsLimit: limit })
      } catch (error) {
   ...
   //
   //          paginatedProducts = await Product.find().limit(limit).skip(skip)
   ...
   //          paginatedProducts = await Product.find({tagsIds: {$in: filteredTagsId}}).limit(limit).skip(skip)
   //
   //          // Получаем весь массив продуктов по фильтру
   //          filteredProducts = await Product.find({tagsIds: {$in: filteredTagsId}})
   //
   ...
   //       }
   //
   //       return response.json({ allTagTypes, allTags, allProducts, paginatedProducts, filteredProducts, productsTotalCount, productsLimit: limit })
   //    } catch (error) {
   //       return response.status(500).json({message: `${error.message}`})
   //    }
   // }
